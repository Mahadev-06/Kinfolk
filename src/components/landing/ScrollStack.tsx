...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  // Cache original positions to prevent feedback loop from getBoundingClientRect
  ...
  const updateCardTransforms = useCallback(() => {
    if (!cardsRef.current.length || !originalOffsetsRef.current.length) return;
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      // Use cached original position — NOT live getBoundingClientRect
      ...
      if (hasChanged) {
        card.style.transform = `translate3d(0, ${newTransform.translateY}px, 0) scale(${newTransform.scale})${newTransform.rotation ? ` rotate(${newTransform.rotation}deg)` : ''}`;
  ...
  useLayoutEffect(() => {
    ...
    // Reset all transforms first so we measure clean positions
    cards.forEach((card) => {
    ...
    cards.forEach((card) => {
      if (useWindowScroll) {
        offsets.push(card.getBoundingClientRect().top + window.scrollY);
      } else {
        offsets.push(card.offsetTop);
      }
    });
    originalOffsetsRef.current = offsets;
    ...
    // Now set GPU hints and z-index stacking
    cards.forEach((card, i) => {
